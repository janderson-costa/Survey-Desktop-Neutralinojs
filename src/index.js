...
function SrvTable() {
  const table = {
    id: "",
    // Válido somente enquanto o arquivo do Excel estiver aberto
    name: "",
    enabled: true,
    rows: [SrvTableRow()]
  };
...
function Utils() {
  ...
  function mergeProps(target2, source) {
    const merged = { ...target2 };
    for (const key in source) {
      if (source[key] instanceof Object && !(source[key] instanceof Array) && !(source[key] instanceof Function) && !(source[key] instanceof HTMLElement)) {
        merged[key] = mergeProps(target2[key] || {}, source[key]);
      } else {
  ...
  function observe(obj, { onChange, onDelete }) {
    ...
    return new Proxy(obj, {
      ...
      set(target2, prop, value, receiver) {
        ...
        const success = Reflect.set(target2, prop, value, receiver);
        if (success && old !== value && onChange) {
          onChange({ target: target2, prop, value });
        }
        return success;
      },
...
// src/services/NeutralinoService.js
...
function NeutralinoService() {
  ...
  function setWindowTitle() {
    ...
    const title2 = config.modes.window.title;
    return Neutralino.window.setTitle(`${title2} - ${version} ${global.appData.state.saved ? "" : "*"}`);
  }
  function setMenubar() {
  }
  ...
  async function openFile2(options = {}) {
    const openResult = await Neutralino.os.open(options.filePath);
    const result = Result();
...
function SrvService() {
  ...
  async function newFile2(options = { minimizeWindow }) {
  ...
  async function openFile2(options = { minimizeWindow }) {
    let result = await neutralinoService.showFileDialog({
      title: "Abrir",
      filters: [
        { name: "Survey", extensions: ["srv"] }
  ...
  async function saveFile2(srvConfig) {
  ...
  planilhas.forEach((planilha) => {
    ...
    data.filter((data2) => data2.Planilha == planilha).forEach((item) => {
      if (item.Grupo && item.Grupo != lastGroup) {
        ...
        const row2 = SrvTableRow();
        row2.isGroup = true;
        row2.name = item.Grupo;
...
async function newFile() {
  ...
  _globalProxy.appData.state.saved = false;
  _globalProxy.appData.srvConfig = result.data;
  start();
}
async function openFile() {
  ...
  result = await srvService.openFile({
    minimizeWindow: () => Neutralino.window.unminimize()
  });
  ...
  _globalProxy.appData.state.opened = true;
...
async function saveFile(confirm = false) {
  ...
  async function save() {
    let result = await srvService.saveFile(_globalProxy.appData.srvConfig);
    if (result.error) {
      Modal({
        title: "Survey",
...
function createUI() {
  ...
  const _toolbarActionsLeft = [
    { title: "Novo", icon: Icon_default("new"), onClick: () => newFile() },
    { title: "Abrir", icon: Icon_default("open"), onClick: () => openFile() },
    { title: "Salvar", icon: Icon_default("save"), onClick: () => saveFile() },
    { title: "Informa\xE7\xF5es do arquivo", icon: Icon_default("info"), onClick: () => showFileInfo() },
    { divider: true },
    { title: "Carregar dados nas planilhas", icon: Icon_default("load"), onClick: () => console.log("onClick") },
    { title: "Limpar dados das planilhas", icon: Icon_default("clear"), onClick: () => console.log("onClick") },
    { title: "Enviar por E-mail", icon: Icon_default("send"), onClick: () => console.log("onClick") }
  ];
  const _toolbarActionsRight = [
    { title: "Visualizar no dispositivo m\xF3vel", icon: Icon_default("smartphone"), onClick: () => console.log("onClick") }
  ];
  const _toolbarTable = [
    { divider: true },
    { title: "Adicionar grupo", icon: Icon_default("addGroup"), onClick: () => console.log("onClick") },
    { title: "Adicionar item", icon: Icon_default("add"), onClick: () => console.log("onClick") },
    { title: "Mover item selecionado para cima", icon: Icon_default("arrowUp"), onClick: () => console.log("onClick") },
    { title: "Mover item selecionado para baixo", icon: Icon_default("arrowDown"), onClick: () => console.log("onClick") },
    { title: "Excluir item selecionado", icon: Icon_default("close"), onClick: () => console.log("onClick") }
  ];
  ...
  const $tabs = html`
		<div class="tabs flex gap-2">${() => srvConfig.data.tables.filter((x) => x.enabled).map((table, index) => {
    const $tab = html`
  ...
  const $buttonAddTable = html`
    ...
    _menu.options.items = _sheets.map((sheet, index) => {
      ...
      if (table) {
        return {
          icon: table.enabled ? Icon_default("check") : "",
          name: sheet.Name,
      ...
      description: {
        display: ({ row, item, value }) => {
          const $field = html`
            ...
            Utils2().form().field().autoHeight(e.element);
          }}">${value}</textarea>
					`;
          Utils2().form().field().autoHeight($field);
          return $field;
        }
      },
      type: {
        display: ({ row, item, value }) => {
          ...
          }}">${constants_default.TABLE_ROW_FIELD_TYPES.map((type) => (
            /*html*/
            `
